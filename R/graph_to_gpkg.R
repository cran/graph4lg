#' Export a spatial graph to geopackage files
#'
...
#' reference system indicated with the argument crds_crs.}
#' }
#' @param mode Indicates which geopackage files will be created
#' \itemize{
#' \item{If 'mode = 'both'' (default), then two geopackage files are created,
#' one for the nodes and another for the links.}
#' \item{If 'mode = 'node'', a geopackage file is created for the nodes only.}
#' \item{If 'mode = 'link'', a geopackage file is created for the links only.}
#' }
#' @param metrics (not considered if 'mode = 'link'') Logical. Should graph
#' node attributes integrated in the attribute table of the node geopackage
#' files? (default: FALSE)
...
#' in which the geopackage files will be exported. If \code{dir_path = "wd"},
#' then the files are created in the current working directory.
#' @param layer A character string indicating the suffix of the name of
#' the layers to be created.
#' @return Create geopackage files in the directory specified with the parameter
#' 'dir_path'.
#' @export
#' @author P. Savary
#' @examples
...
#' crds <- pts_pop_simul
#' layer <- "graph_dps_gab"
#' graph_to_gpkg(graph = gp, crds = pts_pop_simul, mode = "both",
#'              crds_crs = crds_crs,
#'              layer = "test_fonct",
#'              dir_path = tempdir(),
#'              metrics = FALSE)
...
graph_to_gpkg <- function(graph, crds, mode = "both", crds_crs,
                         layer, dir_path,
                         metrics = FALSE){
  ...
  if(any(c(mode, mode) == c("link", "both"))){
    ...
    if(paste0("link_", layer, ".gpkg") %in% list.files(dir_path)){

      message(paste0("A layer named ", paste0("link_", layer, ".gpkg"),
      ...
      sf::st_write(obj = link_lay,
                   dsn = paste0(dir_path, "/link_", layer, ".gpkg"),
                   delete_layer = TRUE,
                   quiet = TRUE)

      message(paste0("Layer link_", layer, ".gpkg was saved in the ",
  ...
  if(any(c(mode, mode) == c("node", "both"))){
    ...
    if(paste0("node_", layer, ".gpkg") %in% list.files(dir_path)){

      message(paste0("A layer named ", paste0("node_", layer, ".gpkg"),
                     " already exists in the directory ", dir_path,
                     ". Please remove it or use another name"))
      ...
      sf::st_write(obj = node_lay,
                   dsn = paste0(dir_path, "/node_", layer, ".gpkg"),
                   delete_layer = TRUE,
                   quiet = TRUE)

      message(paste0("Layer node_", layer, ".gpkg was saved in the ",
                     "following directory: ", dir_path))
    }
  }
}
